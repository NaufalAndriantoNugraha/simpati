...
export function StatCard({ label, value, icon, color = 'indigo', subtitle }: StatCardProps) {
    ...
    return (
        <div className={`flex items-center gap-4 border border-gray-100 ${c.bg} p-5 shadow-sm`}>
            <div className={`flex h-12 w-12 shrink-0 items-center justify-center ${c.icon}`}>
                {icon}
            </div>
