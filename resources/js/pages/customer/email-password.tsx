...
function ConfirmModal({ title, message, confirmLabel, onConfirm, onClose, processing }: {
...
export default function EmailPassword() {
    ...
    return (
        <StudentLayout active="email-password">
            {showEmailModal && (
                <ConfirmModal title="Konfirmasi Perubahan Email"
            ...
            {showPasswordModal && (
                <ConfirmModal title="Konfirmasi Perubahan Password"
            ...
            <div className="mb-6">
                <h1 className="text-xl font-bold text-gray-900">Email & Password</h1>
                <p className="text-sm text-gray-500">Kelola kredensial akun Anda untuk menjaga keamanan.</p>
            </div>
            ...
            <div className="space-y-5 max-w-2xl">
                ...
                <div className="border border-gray-100 bg-white p-6 shadow-sm">
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); setShowEmailModal(true); }} className="space-y-4">
                        <div>
                            ...
                            <input type="email" required value={emailForm.data.email}
                                onChange={(e) => emailForm.setData('email', e.target.value)}
                                placeholder="Masukkan email baru" className={inputCls} />
                            {emailForm.errors.email && <p className="mt-1 text-xs text-red-500">{emailForm.errors.email}</p>}
                        </div>
                        <button type="submit" disabled={emailForm.processing}
                            className="bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-50 transition">
                ...
                <div className="border border-gray-100 bg-white p-6 shadow-sm">
                    <div className="mb-5 flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center bg-red-50">
