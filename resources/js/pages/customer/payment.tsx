...
function StatusBadge({ status }: { status: 'pending' | 'accepted' | 'rejected' }) {
    ...
    return (
        <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-semibold ring-1 ${c.cls}`}>
            {c.icon} {c.label}
        </span>
    );
}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="w-full max-w-sm bg-white shadow-2xl">
                ...
                <div className="p-6">
                    ...
                    <div className="mb-5 bg-red-50 px-4 py-3">
            ...
            {registrations.length === 0 ? (
                ...
                <div className="space-y-4">
                    {registrations.map((reg) => (
                        <div key={reg.id} className="border border-gray-100 bg-white p-6 shadow-sm">
                                ...
                                {reg.payment && reg.payment.status !== 'rejected' ? (
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button onClick={() => setSelectedReg(reg)}
                                            className="flex items-center gap-1.5 bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-emerald-700 transition">
                                            <Upload size={12} />
                            ...
                            {reg.status !== 'accepted' && (
                                <button onClick={() => setCancelReg(reg)}
                                    className="mt-3 w-full border border-red-200 py-2 text-xs font-semibold text-red-500 hover:bg-red-50 transition">
